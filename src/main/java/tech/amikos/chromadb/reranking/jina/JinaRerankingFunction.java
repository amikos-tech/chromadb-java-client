...
public class JinaRerankingFunction implements RerankingFunction {
    ...
    public List<RerankResult> rerank(String query, List<String> documents) throws EFException {
        ...
        try (Response response = client.newCall(request).execute()) {
            ...
            if (responseBody == null) {
                throw new EFException("Jina rerank failed (model: " + model + "): response body was empty");
            }
            ...
            if (rerankResponse == null || rerankResponse.results == null) {
                throw new EFException("Jina rerank failed (model: " + model + "): response did not contain results");
            }
            ...
            throw new EFException("Jina rerank failed: " + e.getMessage(), e);
    ...
    private String requireApiKey(String model) throws EFException {
        ...
        if (normalized == null || normalized.isEmpty()) {
            throw new EFException("Jina rerank failed (model: " + model + "): API key must not be null or empty");
        }
        return normalized;
    }
    ...
    private void validateInputs(String query, List<String> documents, String model) throws EFException {
        if (query == null) {
            throw new EFException("Jina rerank failed (model: " + model + "): query must not be null");
        }
        if (documents == null) {
            throw new EFException("Jina rerank failed (model: " + model + "): documents must not be null");
        }
        if (documents.isEmpty()) {
            throw new EFException("Jina rerank failed (model: " + model + "): documents must not be empty");
        }
    }
}
