...
!!! note "Requires Chroma >= 1.5"
    The Search API is available on Chroma Cloud and on self-hosted Chroma `>= 1.5`.

    For older self-hosted deployments, use `collection.query()` for basic similarity queries - see
    [Records](records.md).
